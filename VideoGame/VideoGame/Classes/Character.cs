...
namespace VideoGame.Classes {
    ...
    public class Character : IAnimatable, ITimer {
        ...
        public void Movement(KeyboardState cur) {
            if (Controllable) {
                if (cur.IsKeyDown(Settings.moveUp) || cur.IsKeyDown(Keys.Up)) { Position.Y -= MovementSpeed; }
                if (cur.IsKeyDown(Settings.moveDown) || cur.IsKeyDown(Keys.Down)) { Position.Y += MovementSpeed; }
                if (cur.IsKeyDown(Settings.moveLeft) || cur.IsKeyDown(Keys.Left)) { Position.X -= MovementSpeed; }
                if (cur.IsKeyDown(Settings.moveRight) || cur.IsKeyDown(Keys.Right)) { Position.X += MovementSpeed; }
            }
        }
